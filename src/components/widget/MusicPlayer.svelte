...
console.log('[MusicPlayer.svelte] Config loaded:', musicPlayerConfig);
console.log('[MusicPlayer.svelte] Playlist:', musicPlayerConfig?.playlist);
...
let duration = 0;
// 音量，默认为 0.7
let volume = musicPlayerConfig?.defaultVolume ?? 0.7;
// 是否静音，默认为 false
let isMuted = false;
// 是否正在加载，默认为 false
let isLoading = false;
...
onMount(() => {
	...
	if (!musicPlayerConfig?.enable) {
		console.log('[MusicPlayer.svelte] Player disabled');
		return;
	}

	if (mode === "meting") {
		console.log('[MusicPlayer.svelte] Using meting mode');
		fetchMetingPlaylist();
	} else {
		// 使用本地播放列表
		console.log('[MusicPlayer.svelte] Using local mode, playlist:', musicPlayerConfig?.playlist);
		playlist = musicPlayerConfig?.playlist ? [...musicPlayerConfig.playlist] : [];
		if (playlist.length > 0) {
			console.log('[MusicPlayer.svelte] Loading first track:', playlist[0]);
			loadSong(playlist[0]);
		} else {
			showErrorMessage("播放列表为空");
		}
	}
});
